...
def get_inc(edge_index):
    ...
    row = torch.cat([torch.arange(edge_num), torch.arange(edge_num)]).to(device)
    col = torch.cat([row_index, col_index]).to(device)
    value = torch.cat([torch.ones(edge_num), -1 * torch.ones(edge_num)]).to(device)
...
def check_inc(edge_index, inc):
    nnz = edge_index.nnz()
    deg = torch.eye(edge_index.sizes()[0]).to(device)
    adj = edge_index.to_dense()
    lap = (inc.t() @ inc).to_dense()
    lap2 = deg - adj
    diff = torch.sum(torch.abs(lap2 - lap)) / nnz
    #print("nnz:",torch.sum(torch.abs(lap2 - lap)),nnz,diff)
        ...
        if self.normalize:
            ...
            if cache is None:
                ...
                if self.cached:
                    ...
                    self.init_z = torch.zeros((inc_mat.sizes()[0], x.size()[-1])).to(device)
            else:
